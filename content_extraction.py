...
Usage:  content.py [-v] -c <content image> [-l <layer name>] [-s <update steps]>
...
def main(argv):
    ...
    try:
        ...
        print('content.py [-v] -c <content image> [-l <layer name>] [-s <update steps>]')
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            print('USAGE: content.py [-v] -c <content image> [-l <layer name>] [-s <update steps>]')
            sys.exit()
